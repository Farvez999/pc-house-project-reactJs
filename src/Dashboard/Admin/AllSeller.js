...
const AllSeller = () => {
    ...
    const { data: sellers = [], refetch } = useQuery({
        queryKey: ['users'],
        queryFn: async () => {
            const res = await fetch(`https://used-products-resale-server-vert.vercel.app/allUser/Seller`);
    ...
    const handleDeleteDoctor = seller => {
        fetch(`https://used-products-resale-server-vert.vercel.app/sellers/${seller._id}`, {
            method: 'DELETE',
            headers: {
    ...
    const handleMakeVerify = id => {
        fetch(`https://used-products-resale-server-vert.vercel.app/users/seller/${id}`, {
            method: 'PUT',
            headers: {
                authorization: `bearer ${localStorage.getItem('accessToken')}`
            }
